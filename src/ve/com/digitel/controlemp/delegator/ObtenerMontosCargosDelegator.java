...
/**
 * Clase ObtenerMontosCargosDelegator implementada con el Patron Singlenton que se encarga de realizar la invocacion al Procedure: PROCESO_PKG.obtenerMontosCargos.
 * 
 * @author Diego Leal - BSS - CC Areas Tecnicas - Integracion
 * @author Rolando Castaneda - FYCCORP - Consultor de Sistema
 */
public class ObtenerMontosCargosDelegator {

	private static Logger logger = BSSIntLogger.getBSSIntLogger(ObtenerMontosCargosDelegator.class);
	...
	private String obtenerMontosCargosProcedure;

	private static ObtenerMontosCargosDelegator rtbs = new ObtenerMontosCargosDelegator();

	/**
	 * Constructor de la Clase.
	 */
	public ObtenerMontosCargosDelegator() {
		...
		try {
			...
			obtenerMontosCargosProcedure = rsr.getString("application.controlemp.procedure.obtenermontoscargos");
	 ...
	 * 
	 * @return ObtenerMontosCargosDelegator Instancia de la Clase.
	 */
	public static ObtenerMontosCargosDelegator getInstance() {
		return rtbs;
	}
		...
		try {
			...
			if (conn != null) {
				...
				try {
					...
					cs = conn.prepareCall("{ call ".concat(obtenerMontosCargosProcedure).concat(" }"));
					...
					map.put("conn", conn);
					map.put("cs", cs);
					returnValue = new ReturnValue("0", "Procedure: ".concat(obtenerMontosCargosProcedure).concat(
							" ejecutado con exito"), "", map);

					logger.info("Resultado Obtener Montos Cargos: " + returnValue.getCondCode());
				} catch (Exception e) {
					logger.error("Error ocurrido en el delegador ObtenerMontosCargosDelegator", e);

					returnValue = new ReturnValue(ConstantMessageCode.EXCEPTION,
							"Error ocurrido en el delegador ObtenerMontosCargosDelegator ".concat(e.getMessage()), "", new HashMap());
				}
			} else {
