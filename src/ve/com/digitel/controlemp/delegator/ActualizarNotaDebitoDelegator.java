...
/**
 * Clase ActualizarNotaDebitoDelegator implementada con el Patron Singlenton que se encarga de realizar la invocacion al Procedure: PROCESO_PKG.actualizarNotaDebito.
 * 
 * @author Diego Leal - BSS - CC Areas Tecnicas - Integracion
 * @author Rolando Castaneda - FYCCORP - Consultor de Sistema
 */
public class ActualizarNotaDebitoDelegator {

	private static Logger logger = BSSIntLogger.getBSSIntLogger(ActualizarNotaDebitoDelegator.class);
	...
	private String actualizarNotaDebitoProcedure;

	// TODO verificar este valor
	private String referenciaIn = "";

	private static ActualizarNotaDebitoDelegator rtbs = new ActualizarNotaDebitoDelegator();

	/**
	 * Constructor de la Clase.
	 */
	public ActualizarNotaDebitoDelegator() {
		try {
			...
			actualizarNotaDebitoProcedure = rsr.getString("application.controlemp.procedure.actualizarnotadebito");
	 ...
	 */
	public static ActualizarNotaDebitoDelegator getInstance() {
		return rtbs;
	}
		...
		try {
			...
			if (conn != null) {
				...
				try {
					...
					cs = conn.prepareCall("{ call ".concat(actualizarNotaDebitoProcedure).concat(" }"));

					cs.setDate(1, date);
					cs.setString(2, referenciaIn);
					cs.setInt(3, ((Integer) parameters.get("idContrato")).intValue());
					cs.setInt(4, ((Integer) parameters.get("idProducto")).intValue());
					cs.setInt(5, ((Integer) parameters.get("idEstadoDebito")).intValue());
					cs.execute();

					returnValue = new ReturnValue("0", "Procedure: ".concat(actualizarNotaDebitoProcedure).concat(
							" ejecutado con exito"), "", new HashMap());

					logger.info("Resultado Actualizar Nota Debito: " + returnValue);
				} catch (Exception e) {
					logger.error("Error ocurrido en el delegador actualizarnotadebitoDelegator", e);

					returnValue = new ReturnValue(ConstantMessageCode.EXCEPTION,
							"Error ocurrido en el delegador actualizarnotadebitoDelegator", "", new HashMap());
				}
			} else {
