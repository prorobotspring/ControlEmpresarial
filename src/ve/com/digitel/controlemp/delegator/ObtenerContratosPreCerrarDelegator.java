...
/**
 * Clase ObtenerContratosPreCerrarDelegator implementada con el Patron Singlenton que se encarga de realizar la invocacion al Procedure: PROCESO_PKG.obtenerContratosPorPreCerrar.
 * 
 * @author Diego Leal - BSS - CC Areas Tecnicas - Integracion
 * @author Rolando Castaneda - FYCCORP - Consultor de Sistema
 */
public class ObtenerContratosPreCerrarDelegator {
	...
	private String obtenerContratosPorPreCerrar;

	private static ObtenerContratosPreCerrarDelegator rtbs = new ObtenerContratosPreCerrarDelegator();

	/**
	 * Constructor de la Clase.
	 */
	public ObtenerContratosPreCerrarDelegator() {
		try {
			...
			obtenerContratosPorPreCerrar = rsr.getString("application.controlemp.procedure.obtenerContratosPorPreCerrar");
	 ...
	 * 
	 * @return ObtenerContratosPreCerrarDelegator Instancia de la Clase.
	 */
	public static ObtenerContratosPreCerrarDelegator getInstance() {
		return rtbs;
	}
	...
	/**
	 * Funcion Encargada de realizar la invocacion al Procedure.<br>
	 * Se actualizo la logica del Procedure para colocar con estado valido las cuentas que ha sido Recuperadora pero tiene GSM afiliados. 
	 * 
	 *
		...
		try {
			...
			if (conn != null) {
				...
				try {
					...
					cs = conn.prepareCall("{ call ".concat(obtenerContratosPorPreCerrar).concat(" }"));
					...
					map.put("conn", conn);
					map.put("cs", cs);
					returnValue = new ReturnValue("0", "Procedure: ".concat(obtenerContratosPorPreCerrar).concat(" ejecutado con exito"), "", map);
