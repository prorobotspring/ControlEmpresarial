...
/**
 * Clase IsDirectNRCDelegator implementada con el Patron Singlenton que se encarga de realizar la invocacion al Procedure: PROCESO_PKG.isredirectnrc.
 * 
 * @author Diego Leal - BSS - CC Areas Tecnicas - Integracion
 * @author Rolando Castaneda - FYCCORP - Consultor de Sistema
 */
public class IsDirectNRCDelegator {

	private static Logger logger = BSSIntLogger.getBSSIntLogger(IsDirectNRCDelegator.class);
	...
	private String isDirectNRCProcedure;

	private static IsDirectNRCDelegator rtbs = new IsDirectNRCDelegator();

	/**
	 * Constructor de la Clase.
	 */
	public IsDirectNRCDelegator() {
		try {
			...
			isDirectNRCProcedure = rsr.getString("application.controlemp.procedure.isredirectnrc");
	 ...
	 * 
	 * @return IsDirectNRCDelegator Instancia de la Clase.
	 */
	public static IsDirectNRCDelegator getInstance() {
		return rtbs;
	}
		...
		try {
			...
			if (conn != null) {
				...
				try {
					...
					cs = conn.prepareCall("{ call ".concat(isDirectNRCProcedure).concat(" }"));

					cs.setInt(1, Integer.parseInt((String) parameters.get("idCuenta")));
					cs.registerOutParameter(2, OracleTypes.NUMBER);
					cs.execute();

					int isRedirect = cs.getInt(2);

					Map map = new HashMap();
					map.put("isRedirect", new Integer(isRedirect));

					returnValue = new ReturnValue("0", "Procedure: ".concat(isDirectNRCProcedure).concat(" ejecutado con exito"),
							"", map);

					logger.info("Resultado IsRedirectNRC: " + returnValue);
				} catch (Exception e) {
					logger.error("Error ocurrido en el delegador isDirectNRCProcedure", e);

					returnValue = new ReturnValue(ConstantMessageCode.EXCEPTION,
							"Error ocurrido en el delegador isDirectNRCProcedure", "", new HashMap());
				}
			} else {
